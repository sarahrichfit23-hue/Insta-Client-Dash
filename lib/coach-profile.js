...
export function buildCoachProfilePrompt(profile) {
  ...
  const deliveryMethod = {
    ...
    'none': 'NOT PROVIDED YET — coach does not have a lead magnet currently'
  }[profile.lead_magnet_delivery] || 'NOT PROVIDED YET'
  ...
  }[profile.offer_sales_method] || 'NOT PROVIDED YET'
  
  // Check if lead magnet section is filled
  const hasLeadMagnet = profile.lead_magnet_name && profile.lead_magnet_name.trim() && profile.lead_magnet_delivery && profile.lead_magnet_delivery !== 'none'
